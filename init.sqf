    ...
    [_volume, _showIntro, _showPlayerMapAndCompass, _isJipPlayer, _useRevive] spawn {
        ...
        if (_isJipPlayer) then {
            ...
            if (_showIntro) then {            
                ...
                ["<t size='1'>" + "Escape Stratis" + "</t>",0.02,0.3,3,-1,0,3011] spawn bis_fnc_dynamicText;
                ["<t size='0.6'>" + "A port of 'Escape Chernarus'" + "</t>",0.02,0.4,3,-1,0,3012] spawn bis_fnc_dynamicText;
				["<t size='0.5'>" + "by [TGH] Vormulac and Hyperz" + "</t>",0.02,0.5,3,-1,0,3013] spawn bis_fnc_dynamicText;
                ...
                0 cutText ["", "BLACK FADED"];
                ["Somewhere on", "Stratis", str (date select 2) + "/" + str (date select 1) + "/" + str (date select 0) + " " + str (date select 3) + ":00"] spawn BIS_fnc_infoText;
            };
